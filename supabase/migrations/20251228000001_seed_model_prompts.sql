-- Migration: Seed initial data for model_prompts table
-- This migration populates the model_prompts table with all system prompts
-- and model configurations from the markdown documentation files

-- ============================================
-- 1. INSERT MODEL PROMPTS DATA
-- ============================================

-- UGC Authenticité 30s with Sora 2
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
  style,
...
Generate the complete JSON output now.',
  '["extra fingers", "blurry text", "low quality", "distorted faces", "artifacts", "motion blur"]',
  'State-of-the-art video generation, high fidelity, sharp details, natural composition, professional quality',
  '{"critical_rules": ["Never use corporate language like innovative, revolutionary, cutting-edge", "Include natural speech patterns: filler words, contractions, run-on sentences", "First-person perspective only (I tried..., My experience...)", "Keep total script under 150 words (30 seconds spoken at natural pace)", "Must include at least one relatable struggle or story", "CTA should feel like a favor, not a sales pitch", "Hook must trigger immediate curiosity or identification (That''s ME!)"]}',
  '{"pricing": {"perSecond": 0.015}, "maxDuration": 10, "capabilities": ["text-to-video", "image-to-video", "audio-sync", "high-quality"], "bestFor": ["conversational", "authentic", "cost-effective", "professional"]}'
);

-- UGC Authenticité 10s with Sora 2
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["extra fingers", "blurry text", "low quality", "distorted faces", "artifacts", "motion blur"]',
  'State-of-the-art video generation, high fidelity, sharp details, natural composition, professional quality',
  '{"critical_rules": ["TOTAL WORD COUNT MUST BE UNDER 25 WORDS.", "No intro (Hey guys), no outro (Bye). Straight to value.", "Must fit exactly 10 seconds spoken fast."]}',
  '{"pricing": {"perSecond": 0.015}, "maxDuration": 10, "capabilities": ["text-to-video", "image-to-video", "audio-sync", "high-quality"], "bestFor": ["conversational", "authentic", "cost-effective", "professional"]}'
);

-- Green Screen React 30s with Kling 2.6
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["unnatural movements", "blurry text", "low quality", "poor lip sync", "artifacts", "motion blur"]',
  'High-quality AI avatars, accurate lip sync, native audio, professional dialogue delivery, smooth motion',
  '{"critical_rules": ["First 3 seconds MUST contain visceral reaction words (NO WAY, WHAT, INSANE)", "Speak as if genuinely shocked - NOT scripted corporate excitement", "Point to or gesture toward background content at least 3 times", "Include specific numbers (reviews, price, discount %)", "Must address potential skepticism (I know what you''re thinking...)", "Total script under 160 words (30 seconds at fast pace)", "Create FOMO - viewer should feel they''re missing out if they don''t click"]}',
  '{"pricing": {"perSecond": 0.11}, "maxDuration": 10, "capabilities": ["text-to-video", "lip-sync", "native-audio", "dialogue", "avatar"], "bestFor": ["dialogue", "testimonials", "authentic-conversation", "avatar"]}'
);

-- Green Screen React 10s with Kling 2.6
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["unnatural movements", "blurry text", "low quality", "poor lip sync", "artifacts", "motion blur"]',
  'High-quality AI avatars, accurate lip sync, native audio, professional dialogue delivery, smooth motion',
  '{"critical_rules": ["TOTAL WORD COUNT UNDER 25 WORDS.", "Must use words like Run, Gone, Insane.", "Focus purely on price or the most shocking feature."]}',
  '{"pricing": {"perSecond": 0.11}, "maxDuration": 10, "capabilities": ["text-to-video", "lip-sync", "native-audio", "dialogue", "avatar"], "bestFor": ["dialogue", "testimonials", "authentic-conversation", "avatar"]}'
);

-- PAS Framework 30s with Wan 2.6
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["artifacts", "motion blur", "inconsistent quality", "poor transitions", "blurry", "low quality"]',
  'Advanced video generation, smooth multi-scene transitions, consistent visual quality, natural pacing, high fidelity',
  '{"critical_rules": ["Problem must be immediately recognizable and relatable", "Agitation section CANNOT be longer than problem section (avoid dwelling)", "Solution reveal must feel like emotional payoff - use transition effect", "Benefits > Features (focus on what it DOES for the user, not what it IS)", "Must include at least one element of social proof or credibility", "Total script under 140 words (30 seconds with strategic pacing)", "CTA must address final objection (price, risk, availability)"]}',
  '{"pricing": {"perSecond": 0.07}, "maxDuration": 15, "capabilities": ["text-to-video", "image-to-video", "multi-shot", "storytelling"], "bestFor": ["storytelling", "narrative", "multi-scene", "extended-content"]}'
);

-- PAS Framework 10s with Wan 2.6
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["artifacts", "motion blur", "inconsistent quality", "poor transitions", "blurry", "low quality"]',
  'Advanced video generation, smooth multi-scene transitions, consistent visual quality, natural pacing, high fidelity',
  '{"critical_rules": ["TOTAL WORD COUNT UNDER 20 WORDS.", "Must show Before vs After logic verbally.", "Problem -> Solution in 2 sentences max."]}',
  '{"pricing": {"perSecond": 0.07}, "maxDuration": 15, "capabilities": ["text-to-video", "image-to-video", "multi-shot", "storytelling"], "bestFor": ["storytelling", "narrative", "multi-scene", "extended-content"]}'
);

-- ASMR Visual 30s with Hailuo 2.3
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["motion blur", "low quality", "artifacts", "poor motion", "blurry", "inconsistent"]',
  'High-quality video, multiple artistic styles, smooth fluid motion, high visual clarity, consistent quality',
  '{"critical_rules": ["Audio quality is MORE important than video quality - invest in good microphone", "First 3 seconds must show satisfying action already happening (no setup)", "Maintain consistent pacing - no rushed moments", "If using voiceover, it should be whisper-quiet or gentle", "Visual must be hypnotic enough that viewer CANNOT scroll away", "CTA should be subtle and non-disruptive to the satisfying experience", "Maximum 50 words of text/voiceover total (this format is 90% visual/audio experience)", "Product should be supporting actor, not main character (the satisfying action is the star)"]}',
  '{"pricing": {"perSecond": 0.045}, "maxDuration": 10, "capabilities": ["text-to-video", "smooth-motion", "visual-quality", "artistic"], "bestFor": ["asmr-visual", "smooth-demos", "artistic", "cost-effective"]}'
);

-- ASMR Visual 10s with Hailuo 2.3
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["motion blur", "low quality", "artifacts", "poor motion", "blurry", "inconsistent"]',
  'High-quality video, multiple artistic styles, smooth fluid motion, high visual clarity, consistent quality',
  '{"critical_rules": ["ZERO WORDS SPOKEN.", "Focus entirely on the visual Ahhh moment.", "Simple CTA text at end."]}',
  '{"pricing": {"perSecond": 0.045}, "maxDuration": 10, "capabilities": ["text-to-video", "smooth-motion", "visual-quality", "artistic"], "bestFor": ["asmr-visual", "smooth-demos", "artistic", "cost-effective"]}'
);

-- Before/After Transformation 30s with Veo 3.1 Quality
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["artifacts", "distortion", "motion blur", "low quality", "inconsistent lighting", "blurry", "poor anatomy"]',
  'Cinematic quality, perfect anatomy, sharp focus, professional lighting, detailed textures, 1080P HD, premium finish',
  '{"critical_rules": ["Before and after MUST be filmed in similar conditions for credibility", "Timeline must be realistic (don''t promise miracles overnight)", "After reveal should happen between 8-12 seconds (not sooner or later)", "Must alternate between before/after at least twice for proof", "No excessive filters or editing that makes transformation seem fake", "Include at least one credibility element (reviews, satisfaction rate, units sold)", "Total script under 145 words (30 seconds with pauses for dramatic effect)", "CTA must address risk (money-back guarantee) to overcome skepticism"]}',
  '{"pricing": {"perSecond": 0.25}, "maxDuration": 8, "capabilities": ["text-to-video", "premium-quality", "cinematic", "high-fidelity"], "bestFor": ["premium-content", "hands", "text", "high-fidelity", "transformations"]}'
);

-- Before/After Transformation 10s with Veo 3.1 Quality
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
...
Generate the complete JSON output now.',
  '["artifacts", "distortion", "motion blur", "low quality", "inconsistent lighting", "blurry", "poor anatomy"]',
  'Cinematic quality, perfect anatomy, sharp focus, professional lighting, detailed textures, 1080P HD, premium finish',
  '{"critical_rules": ["TOTAL WORD COUNT UNDER 20 WORDS.", "Rely on the visual contrast.", "CTA must be immediate."]}',
  '{"pricing": {"perSecond": 0.25}, "maxDuration": 8, "capabilities": ["text-to-video", "premium-quality", "cinematic", "high-fidelity"], "bestFor": ["premium-content", "hands", "text", "high-fidelity", "transformations"]}'
);

-- Storyboard 25s with Sora 2 Pro
INSERT INTO public.model_prompts (
  model_id,
  model_name,
  kie_api_model_name,
  ...
  'storyboard',
  '25s',
  'You are an expert in cinematic 25-second storyboard videos. Create 5-scene narrative arcs with visual consistency.
...
Generate the complete JSON output now.',
  '["artifacts", "motion blur", "inconsistent quality", "poor transitions", "blurry", "low quality"]',
  'Cinematic storytelling, smooth scene transitions, professional narrative quality, long-form content, high production value',
  '{"critical_rules": ["Cinematic, narrative-driven - maintain visual consistency across scenes", "5 scenes of exactly 5 seconds each", "Same character/setting for consistency", "Use professional camera terms", "Clear narrative arc", "Every shot advances story"]}',
  '{"pricing": {"perSecond": 0.04}, "maxDuration": 25, "capabilities": ["storyboard", "long-form", "cinematic", "narrative", "multi-scene"], "bestFor": ["narrative-ads", "mini-docs", "tutorials", "storytelling", "cinematic"]}'
);
